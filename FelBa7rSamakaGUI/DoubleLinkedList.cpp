...
void DoubleLinkedList::add(CreatureRLE* k, int num)
{
    if(head == NULL)
    {
...
void DoubleLinkedList::add(CreatureRLE* k, int num, int index)
{
...
DoubleLinkedListNode* DoubleLinkedList::getHead()
{
    return head;
}
...
void DoubleLinkedList::print()
{
    ...
    for(int i=0; i<Size; i++)
    {
        printf("Type: %d   StarveTime: %d   Number: %d %d\n", current->getCreatureRLE()->gettype(), current->getCreatureRLE()->getStarve(), current->getNumber(), current->getCreatureRLE()->getPosition());
        current = current->getNext();
    }
}
